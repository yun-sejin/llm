# 아래 예시는 test_data_chunks 테이블에서 src_id를 기본 키로 설정하고, 각 레코드의 data를 청크 단위로 나누어 PostgreSQL에 삽입하는 방식으로 수정한 코드입니다. 각 데이터의 크기를 바이트 단위로 계산하고 출력한 후, 이를 청크 단위로 PostgreSQL에 삽입합니다.
...
def insert_data_in_chunks(data_list, chunk_size=1024 * 1024):  # 1MB 청크
    ...
    for record in data_list:
        ...
        for i in range(0, len(data), chunk_size):
            ...
            print('chunk:', chunk)
            cur.execute(
                "INSERT INTO test_data_chunks (src_id, data, name, age, email, address) VALUES (%s, %s, %s, %s, %s, %s::jsonb) ON CONFLICT (src_id) DO UPDATE SET data = excluded.data",
...
# ON CONFLICT (src_id) DO UPDATE SET data = excluded.data를 사용하여 src_id가 중복될 경우 data를 업데이트합니다.
# 실행 순서:
