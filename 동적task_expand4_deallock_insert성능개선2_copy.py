...
#copy
    ...
    # 기존의 Python 코드에서 PostgreSQL의 COPY 명령을 사용하여 dsllm_raw 테이블에 데이터를 삽입하는 방식으로 변경하려면, psycopg2 라이브러리를 사용하여 COPY 명령을 실행하는 함수를 추가해야 합니다. COPY 명령은 파일이나 표준 입력으로부터 직접 데이터를 삽입할 때 매우 빠른 성능을 제공합니다.

    # 아래는 psycopg2를 사용하여 COPY 명령을 실행하는 예시 코드입니다. 이 코드는 upload_raw 함수 내에서 호출되어야 합니다. upload_raw 함수는 job_id를 인자로 받아 해당 job_id에 해당하는 데이터 파일을 dsllm_raw 테이블에 삽입하는 로직을 포함해야 합니다.

                    # import psycopg2

                    # def copy_data_to_dsllm_raw(conn_info, file_path):
                    #     """
                    #     PostgreSQL의 COPY 명령을 사용하여 dsllm_raw 테이블에 데이터를 삽입합니다.
                    #     :param conn_info: 데이터베이스 연결 정보 문자열
                    #     :param file_path: 삽입할 데이터가 있는 파일의 경로
                    #     """
                    #     try:
                    #         with psycopg2.connect(conn_info) as conn:
                    #             with conn.cursor() as cur:
                    #                 # COPY 명령 실행
                    #                 copy_sql = """
                    #                 COPY dsllm_raw FROM stdin WITH CSV HEADER DELIMITER AS ','
                    #                 """
                    #                 with open(file_path, 'r') as f:
                    #                     cur.copy_expert(sql=copy_sql, file=f)
                    #                 print(f"Data successfully copied from {file_path} to dsllm_raw table.")
                    #     except Exception as e:
                    #         print(f"Error copying data to dsllm_raw table: {e}")

                    # # 예시: copy_data_to_dsllm_raw 함수 사용
                    # conn_info = "dbname='your_db' user='your_user' host='your_host' password='your_password'"
                    # file_path = "/path/to/your/data_file.csv"
                    # copy_data_to_dsllm_raw(conn_info, file_path)
    # 이 코드는 다음 작업을 수행합니다:

    # psycopg2.connect를 사용하여 PostgreSQL 데이터베이스에 연결합니다.
    # conn.cursor()를 사용하여 커서 객체를 생성합니다.
    # COPY 명령을 실행하여 file_path에서 지정한 파일의 내용을 dsllm_raw 테이블에 삽입합니다. 이 예시에서는 CSV 파일 형식을 가정하고 있습니다.
    # 파일을 열고 cur.copy_expert 메서드를 사용하여 COPY 명령을 실행합니다.
    # 이 방법을 사용하면 대량의 데이터를 매우 빠르게 dsllm_raw 테이블에 삽입할 수 있습니다. COPY 명령은 대량의 데이터를 처리할 때 일반적인 INSERT 명령보다 훨씬 더 효율적입니다.

    #  SON 파일을 읽어 CSV 파일로 변환하는 Python 스크립트를 작성하는 과정은 다음과 같습니다:

    # JSON 파일 읽기: json 모듈을 사용하여 JSON 파일을 읽고 파싱합니다.
    # CSV 파일 생성: csv 모듈을 사용하여 CSV 파일을 생성하고 데이터를 쓰기 위한 준비를 합니다.
    # 데이터 변환 및 쓰기: JSON 데이터를 순회하면서 필요한 데이터를 추출하고 CSV 형식으로 변환하여 파일에 쓰기를 수행합니다.
    # JSON 파일을 CSV로 변환하는 Python 코드 예시
    ...
    #         # CSV 헤더 쓰기 (예시에서는 JSON 객체의 키를 헤더로 가정)
    #         if data:
    #             writer.writerow(data[0].keys())

    #         # JSON 데이터를 순회하며 CSV 파일에 쓰기
    #         for item in data:
    #             writer.writerow(item.values())

    # 이 코드는 JSON 파일(data.json)을 읽고, 각 객체의 키를 CSV 파일(data.csv)의 헤더로 사용합니다. 그리고 JSON 객체의 값을 CSV 파일의 각 행으로 쓰게 됩니다. JSON 파일의 구조가 배열 형태로 되어 있고, 배열의 각 요소가 CSV로 변환될 객체라고 가정합니다.

    # JSON과 CSV 파일의 구조에 따라, 특히 JSON 객체의 중첩 레벨이나 배열 구조에 따라, 이 코드를 적절히 조정해야 할 수도 있습니다.
